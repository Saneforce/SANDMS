...
import static com.saneforce.dms.utils.Shared_Common_Pref.PREF_KEY_APP_AUTO_START;
...
public class AutoStartHelper {
    ...
    private String PACKAGE_OPPO_MAIN_ACTIVITY = "com.coloros.privacypermissionsentry.PermissionTopActivity";
    ...
    private void autoStartAsus(final Context context) {
        if (isPackageExists(context, PACKAGE_ASUS_MAIN)) {
            ...
            showAlert(context, (dialog, which) -> {
                try {
                    shared_common_pref.save(PREF_KEY_APP_AUTO_START, true);
...
//        if (isPackageExists(context, PACKAGE_OPPO_MAIN) || isPackageExists(context, PACKAGE_OPPO_FALLBACK) || isPackageExists(context, PACKAGE_OPPO_MAIN_ACTIVITY)) {
            showAlert(context, new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialog, int which) {
